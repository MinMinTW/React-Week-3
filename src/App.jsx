...
function App() {
  ...
  useEffect(() => {
    ...
    checkUserLogin();//登入渲染時呼叫checkUserLogin這支api
  }, [])
  ...
  useEffect (() => {
    ...
    new Modal(productModalRef.current, {
      backdrop: false //點擊modal背景黑色部分 不會關閉
    });  
  ...
  const handleImageChange = (e, index) => {
    ...
    newImages[index] = value //對到相對應欄位把value帶進去
  ...
  const handleAddImage = () => {
    const newImages = [...tempProduct.imagesUrl, '']; //多給一個空字串
    setTempProduct({
      ...tempProduct,
      imagesUrl: newImages
    })
  ...
  const createProduct = async () => {
    try{
      await axios.post(`${BASE_URL}/v2/api/${API_PATH}/admin/product`,{
        data:{
        ...tempProduct,
        origin_price: Number(tempProduct.origin_price),//轉成數字
        price: Number(tempProduct.price), is_enabled: tempProduct.is_enabled ? 1 : 0
        }
      })
    }catch(error){
      alert('新增產品失敗')
  ...
  const handleUpdateProduct = async() =>{
    const apiCall = modalMode === 'create' ? createProduct : updateProduct;//先判斷是新增還是刪除
    try {
      await apiCall();
      
      getProducts();

      handleCloseProductModal()//新增完就關閉
  }catch(error){
    alert('更新產品失敗')
  }
}
                  ...
                  {products.map((product) => (
                    <tr key={product.id}>
                      ...
                      <div className="btn-group">
                        ...
                        <button 
                        onClick={()=>handleOpenDelProductModal(product)}//刪除顯示產品名
                        type="button" className="btn btn-outline-danger btn-sm">刪除</button>
                      </div>
                      </td>
                    </tr>
                  ))}
                ...
                <div className="col-md-4">
                  ...
                  <div className="border border-2 border-dashed rounded-3 p-3">
                    ...
                    <div className="btn-group w-100">
                     {tempProduct.imagesUrl.length < 5 && tempProduct.imagesUrl[tempProduct.imagesUrl.length - 1] !== '' && ( <button onClick={handleAddImage}
                    ...
                    </div>{/* 新增&刪除按鈕 用圖片的長度去判斷是否顯示新增或取消圖片*/}
